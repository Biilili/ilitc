    ...
    : QMainWindow(parent)   
{
   ...
   //connect(menuA->actions().at(4), &QAction::triggered, this, &GISLite::getView);
}
...
void MainWindow::on_action_triggered()
{
    ...
    //QString qstr = "D:/Project/OOPractice/Code/Code2/polygon.txt";
    ...
    for (int j = 0; j < filename.size(); j++)
    {
        ...
        if (filename.back().right(3) == "txt")
          //  F.getData(filename[j].toStdString().c_str(), geom);
      //  else if (filename.back().right(3) == "shp")
          //  F.ShpToData(filename[j].toStdString().c_str(), geom);
