...
public:
    ...
    {
        if (selectView != -1 && (this == currentLayer->getVector()[selectView]))
        ...
        painter.drawEllipse((*p[0]).x, (*p[0]).y, 16, 16);
    }
    void create(double x, double y)
    {
        QVector<Point*>* newpointvector = new QVector<Point*>();
        View* newpointview = new PointView(*newpointvector);
        (currentLayer->getVector()).push_back(newpointview);
        (currentLayer->getVector()).last()->get_p().push_back(new Point(x, y));
    }
    BBox get_box()
    {
        return GetBBox();
...
public:
    ...
    void show(QPainter& painter)
    {
        ...
        painter.drawEllipse((*p[0]).x, (*p[0]).y, 6, 6);
        for (int i = 1; i < p.size(); i++)
        {
            painter.drawEllipse((*p[i]).x, (*p[i]).y, 6, 6);
    ...
    void create(double x, double y)
    {
        ...
        {
            QVector<Point*>* newpointvector = new QVector<Point*>();
            View* newpointview = new LineView(*newpointvector);
            (currentLayer->getVector()).push_back(newpointview);
            createStep++;
        }
...
public:
    ...
    void show(QPainter& painter)
    {
        ...
        for (int i = 0; i < p.size(); i++)
        {
            painter.drawEllipse((*p[i]).x, (*p[i]).y, 10, 10);
        }
    ...
    void create(double x, double y)
    {
        ...
        {
            QVector<Point*>* newpointvector = new QVector<Point*>();
            View* newpointview = new PolygonView(*newpointvector);
            (currentLayer->getVector()).push_back(newpointview);
            createStep++;
        }
        (currentLayer->getVector()).last()->get_p().push_back(new Point(x, y));
...
{public:
    ...
    void create(double x, double y)
    {
        ...
        {
            QVector<Point*>* newpointvector = new QVector<Point*>();
            View* newpointview = new RectangleView(*newpointvector);
            (currentLayer->getVector()).push_back(newpointview);
            createStep++;
        }
        else if (createStep == 1)
...
public:
    ...
    void create(double x, double y)
    {
        ...
        {
            QVector<Point*>* newpointvector = new QVector<Point*>();
            View* newpointview = new CircleView(*newpointvector);
            (currentLayer->getVector()).push_back(newpointview);
            createStep++;
        }
        else if (createStep == 1)
...
public:
    ...
    void create(double x, double y)
    {
        ...
        {
            QVector<Point*>* newpointvector = new QVector<Point*>();
            View* newpointview = new SectorView(*newpointvector);
            (currentLayer->getVector()).push_back(newpointview);
            createStep++;
        }
        else if (createStep == 1)
...
protected:
    ...
    void mousePressEvent(QMouseEvent* event)
    {
        if (event->button() == Qt::LeftButton)
        {
            ...
            {
                ppt->mousePressEvent(event->x(), event->y());
                repaint();
            }
        }
    }
    void mouseMoveEvent(QMouseEvent* event)
    {
        ...
        {
            ppt->mouseMoveEvent(event->x(), event->y());
            repaint();
        }
    }
    void mouseReleaseEvent(QMouseEvent* event)
    {
        ...
        {
            ppt->mouseReleaseEvent(event->x(), event->y());
            repaint();
        }
    }
...
private slots:
    ...
    void weizhi(QListWidgetItem* item);
    void getView(QVector<Graphic*> geom, int j);
...
public slots:
    void Invoke()
    {
        ...
        ppt = new ModifyBorderWidthHandle;
    }
};
class Shapefile
{
...
{
        Graphic* ptr = NULL;
        file.open(str.toStdString());
        if (!file)
        {
            qDebug() << "The file open defeat!" << endl; return false;
        }
        qDebug() << "The file: " << str << "open successfully!" << endl;//文件打开成功，参数初始化定义
        ...
        while (std::getline(file, linestring, '\n'))
        {
            ...
            if (typestring == "Point")
            {
                ...
                os >> temp.x >> temp.y ;//读取(x,y)
                p.push_back(new Point(temp.x, temp.y));
                ptr = new CPoint(p, "#000000", 1); //创建点对象
            }
            else if (typestring == "Linestring")
            {
               
                
                while( os >> temp.x >> temp.y)
                {
                    //读取(x,y)
                    p.push_back(new Point(temp.x, temp.y));
                } 
              
                ptr = new CLine(p, 2,"#000000", 1);
            }
            else if (typestring == "Polygon")
            {
               while(os >> temp.x >> temp.y)
                ...
                ptr = new CPolygon(p, 2, "#000000", 1, "#000000", 1);
            }
            else if (typestring == "Rectangle")
            {
               while(os >> temp.x >> temp.y )
              ...
              //  os >> width >> bordercolor >> borderstyle >> fillcolor >> fillstyle;//读取(x,y)
                ptr = new CRectangle(p, 2, "#000000", 1, "#000000", 1);
            }
            else if (typestring == "Circle")
            {
                double r;
                os >> temp.x >> temp.y  >> r ;//读取(x,y)
                p.push_back(new Point(temp.x, temp.y));
               // os >> r >> width >> bordercolor >> borderstyle >> fillcolor >> fillstyle;//读取(x,y)
                ptr = new CCircle(p, r, 2, "#000000", 1, "#000000", 1);
            }
            else if (typestring == "Sector")
            {
                double r;
             ...
             //   os >> r >> width >> bordercolor >> borderstyle >> fillcolor >> fillstyle;//读取(x,y)
                ptr = new CSector(p, r, 2, "#000000", 1, "#000000", 1);
            }
            array.push_back(ptr);
            ptr = NULL;
        }
